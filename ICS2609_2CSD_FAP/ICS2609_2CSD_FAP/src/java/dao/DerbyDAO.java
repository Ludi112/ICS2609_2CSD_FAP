...
public class DerbyDAO 
{
    ...
    /**
     * Retrieves all users formatted for the PDF Report.
     * Appends an asterisk (*) to the currently logged-in admin.
     */
    public List<String[]> getAllUsersForReport(String loggedInAdmin) {
        List<String[]> data = new ArrayList<>();
        final String SQL = "SELECT Username, Role, Created_Date FROM Users ORDER BY Role ASC, Username ASC";
        ...
        try (PreparedStatement ps = conn.prepareStatement(SQL);
            ...
            while (rs.next()) {
                ...
                if (user.equalsIgnoreCase(loggedInAdmin.trim())) {
                    user += " *"; 
                }
            ...
            LOGGER.log(Level.SEVERE, "Derby: getAllUsersForReport error", e);
        }
        return data;
    }

    /**
     * Retrieves only the records related to the specific logged-in admin.
     */
    public List<String[]> getAdminRecordsForReport(String loggedInAdmin) {
        List<String[]> data = new ArrayList<>();
        final String SQL = "SELECT Username, Role, Created_Date FROM Users WHERE TRIM(Username) = ?";
        
        try (PreparedStatement ps = conn.prepareStatement(SQL)) {
            ps.setString(1, loggedInAdmin.trim());
            ...
            LOGGER.log(Level.SEVERE, "Derby: getAdminRecordsForReport error", e);
        }
        return data;
    }
}
