...
public class AuthFilter implements Filter {
    ...
    private boolean isPublicPath(String uri, String contextPath) {
        ...
        if (uri.equals(contextPath + "/login.jsp"))          return true;
        if (uri.equals(contextPath + "/LoginServlet"))        return true;

        // CAPTCHA servlet (must be reachable before login)
        if (uri.equals(contextPath + "/CaptchaServlet"))      return true;

        // Custom error pages (must be reachable by unauthenticated users)
        if (uri.startsWith(contextPath + "/errors/"))         return true;

        // Root index (redirects to login)
        if (uri.equals(contextPath + "/")
                || uri.equals(contextPath + "/index.jsp"))    return true;
        ...
        if (uri.endsWith(".woff"))  return true;
        if (uri.endsWith(".woff2")) return true;
        if (uri.endsWith(".svg"))   return true;
